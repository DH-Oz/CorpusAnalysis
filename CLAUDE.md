...
- **`liwc-dictionary-handling`** — LIWC dictionaries are licensed, in any language, and are handled exactly as `.notes/feedback_liwc-dictionary-handling.md` sets out. A loose `liwcdict.dic` never sits unencrypted in the repo, and `.gitignore` keeps it out. Custom open dictionaries (`nietzsche.dic`, `macdvirtue.dic`, `nuke.dic`, etc.) DO ship publicly and unencrypted.
...
- **Never commit**: any LIWC dictionary as a loose unencrypted file, in any language. See `.notes/feedback_liwc-dictionary-handling.md` before touching dictionary distribution.
