...
def test_cooccurrence_graph_links_cooccurring_categories():
    ...
    # a and b appear together in two documents; c appears alone in one.
    counts = numpy.array([[1, 1, 0], [1, 1, 0], [0, 0, 1]])
    graph = cooccurrence_graph(counts, ["a", "b", "c"], top_edges=40)
    assert set(graph.nodes()) == {"a", "b"}
    assert graph.has_edge("a", "b")
    # weight is the number of documents the pair co-occurs in.
    assert graph["a"]["b"]["weight"] == 2


def test_cooccurrence_graph_removes_self_loops():
    from corpus_tools import cooccurrence_graph

    counts = numpy.array([[1, 1, 0], [1, 1, 0], [0, 0, 1]])
    graph = cooccurrence_graph(counts, ["a", "b", "c"], top_edges=40)
    assert list(networkx.selfloop_edges(graph)) == []


def test_cooccurrence_graph_keeps_only_the_strongest_edges():
    from corpus_tools import cooccurrence_graph

    # a-b co-occur three times; a-c and b-c once each. top_edges=1 keeps only a-b.
    counts = numpy.array(
        [[1, 1, 0], [1, 1, 0], [1, 1, 0], [1, 0, 1], [0, 1, 1]]
    )
    graph = cooccurrence_graph(counts, ["a", "b", "c"], top_edges=1)
    assert graph.has_edge("a", "b")
    assert not graph.has_edge("a", "c")
    assert not graph.has_edge("b", "c")
    # c is left with no edges and is dropped.
    assert set(graph.nodes()) == {"a", "b"}


def test_draw_cooccurrence_network_runs_without_error():
    from corpus_tools import draw_cooccurrence_network

    counts = numpy.array([[2, 1, 0], [1, 2, 1], [0, 1, 2]])
    draw_cooccurrence_network(counts, ["a", "b", "c"], top_edges=3, title="t")
    import matplotlib.pyplot as plt
...
def test_dispersion_plot_runs_without_error():
    ...
    lengths = [100, 80]
    positions = [[3, 40, 90], [10, 60]]
    dispersion_plot(labels, lengths, positions, title="t")
    import matplotlib.pyplot as plt
